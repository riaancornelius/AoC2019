...
// Part 1:
println("Part 1: ${Computer(inputString).startCalculation(updates = *arrayOf(Pair(1, 12), Pair(2, 2)))}")

// Part 2:
val expectedOutput = 19690720
// Need to figure out how to avoid the fact that they are nullable
val noun = (0..99).findLast {
    run {
    val computer = Computer(inputString)
    computer.startCalculation(updates = *arrayOf(Pair(1, it), Pair(2, 0)))
