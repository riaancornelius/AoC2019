...
// Part 1:
println("Part 1: ${Computer(inputString).runCalculation(updates = *arrayOf(Pair(1, 12), Pair(2, 2)))}")

// Part 2:
val expectedOutput = 19690720
// Need to figure out how to avoid the fact that they are nullable
val noun = (0..99).findLast {it -> run {
    val computer = Computer(inputString)
    computer.runCalculation(updates = *arrayOf(Pair(1, it), Pair(2, 0)))
    computer.read(0, Computer.Mode.IMMEDIATE)
} < expectedOutput }
val verb = (0..99).find {
    noun?.let { it1 -> run {
        val computer = Computer(inputString)
        computer.runCalculation(updates = *arrayOf(Pair(1, it1), Pair(2, it)))
        computer.read(0, Computer.Mode.IMMEDIATE)
    } } == expectedOutput
}
println()
